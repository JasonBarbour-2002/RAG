...
similar_indices, similar_scores = cos_sim_numpy(query, embeddings, topk=top_k)
...
ts = time.process_time()
similar_indices, similar_scores = faiss_similarity(query, embeddings, topk=top_k, index=index)
te = time.process_time() - ts
times[1] = te
...
ts = time.process_time()
similar_indices, similar_scores = ivfflat(query, embeddings, topk=top_k, sql=sql)
te = time.process_time() - ts
times[2] = te
...
# Benchmarking hnsw
ts = time.process_time()
similar_indices, similar_scores = hnsw(query, embeddings, topk=top_k, sql=sql)
te = time.process_time() - ts
times[3] = te
